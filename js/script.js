...
$(document).ready(function(){
    ...
    $(".teamItem").on("click", function(){
    	$(this).closest(".teamItemWrap").find(".teamItemDesc").addClass("show");
    });
	...
	var sticky_navigation = function(){
		...
		if (scroll_top > sticky_navigation_offset_top) { 
			$('#header .headerWrap').addClass("is-sticky").css({ 'position': 'fixed', 'top':0, 'left':0 });
		} else {
			$('#header .headerWrap').removeClass("is-sticky").css({ 'position': 'relative' }); 
		}   
	};
	sticky_navigation();
